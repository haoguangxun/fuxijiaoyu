...
use backend\modules\admin\models\search\User as UserSearch;
...
//            ],
    ...
    public function actionIndex()
    {
        ...
        if(Yii::$app->user->id ==1 || Yii::$app->user->identity->roleid ==1) {
            ...
            $roles = Yii::$app->authManager->getRolesNoRoot();
        }
     ...
     * @param unknown $id
     */
    public function actionStatus($id,$op)
    {   
        if( $id != 1 ) {
    ...
    public function actionChangePassword()
    {
        //特殊用户保护        
        if((Yii::$app->user->id !=1 || Yii::$app->user->identity->roleid !=1) &&
            (Yii::$app->getRequest()->get('id') == 1 || Yii::$app->getRequest()->post('id') == 1)) {
            exit;
        }
     ...
     * @param number $id
     */
    public function actionAssignRole($id = 0)
    {
